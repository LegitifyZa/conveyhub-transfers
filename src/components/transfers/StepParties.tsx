...
const StepParties: React.FC = () => {
  ...
  const PartyCard: React.FC<{ party: Party, isPrimary?: boolean }> = ({ party, isPrimary }) => (
    <Card variant="premium" className="animate-in fade-in slide-in-from-bottom-3 duration-300 hover:shadow-premium transition-all duration-300">
      <CardContent className="p-4 space-y-4">
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center space-x-2">
            {parties.filter(p => p.type === party.type).length > 1 && !isPrimary && (
              <Button
                variant="secondary"
                size="sm"
                onClick={() => setPrimaryParty(party.type, party.id)}
          ...
          <div className="mt-3 space-y-2">
            ...
            <div className="flex flex-wrap gap-4 text-xs">
              <div className={`flex items-center space-x-1 ${
                ...
                <span>At least 1 buyer</span>
              </div>
              <div className={`flex items-center space-x-1 ${
                parties.some(p => p.type === 'seller') ? 'text-green-600' : 'text-red-600'
              }`}>
                <span>{parties.some(p => p.type === 'seller') ? '✓' : '○'}</span>
                <span>At least 1 seller</span>
              </div>
              <div className={`flex items-center space-x-1 ${
                parties.every(p => p.name && p.idNumber && p.email) ? 'text-green-600' : 'text-yellow-600'
              }`}>
                <span>{parties.every(p => p.name && p.idNumber && p.email) ? '✓' : '○'}</span>
