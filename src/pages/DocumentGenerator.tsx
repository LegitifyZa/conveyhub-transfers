...
import { Badge, Button, Card, CardContent, CardHeader, CardTitle, UnavailableNotice } from '@/components/ui'
...
const DocumentGenerator: React.FC = () => {
  ...
  const [clausesOffline, setClausesOffline] = useState(false)
  ...
  const loadHistory = async () => {
    try {
      ...
      setHistoryError(err instanceof Error ? err.message : 'Generated document history is temporarily unavailable.')
    }
  }
  ...
  const recordGeneratedDocument = async (entry: GenerationHistoryEntry) => {
    try {
      ...
      setHistoryError('The generated document audit record could not be saved — the document service is temporarily unavailable.')
    }
  }
  ...
  const generateDocument = async () => {
    ...
    try {
      ...
      setGenerationMessage(`${format.toUpperCase()} document generated locally and downloaded. It is not saved to the document service.`)
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      {clausesOffline ? (
        <UnavailableNotice
          ...
          detail="Bundled sample clauses are shown for reference only and generation is local-only — nothing is saved to the server."
        />
      ) : error ? (
      ...
      {isLoading ? (
        ...
        <>
          ...
          <Card><CardHeader><CardTitle className="flex items-center gap-2"><History className="h-5 w-5 text-teal-600 dark:text-teal-400" />Generated Document Audit History</CardTitle></CardHeader><CardContent>{historyError ? <UnavailableNotice message="Generated document history is temporarily unavailable." detail="Any entries below were generated in this session only and are not saved to the server." /> : null}{generationHistory.length === 0 && !historyError ? <p className="text-sm text-gray-500 dark:text-gray-400">No generated documents are recorded.</p> : <div className="space-y-3">{generationHistory.map(entry => <div key={entry.documentId} className="flex flex-col gap-2 border-b border-gray-100 pb-3 text-sm last:border-0 last:pb-0 dark:border-navy-700 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium text-gray-900 dark:text-gray-100">{entry.fileName}.{entry.format.toLowerCase()}</p><p className="text-gray-600 dark:text-gray-400">Matter {entry.matterId} · Template v{entry.templateVersion} · Generator v{entry.generatorVersion}</p><p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Clauses: {entry.clauseVersions.join(', ') || 'None'}</p></div><div className="text-xs text-gray-500 dark:text-gray-400">Generated by {entry.actor}<br />{new Intl.DateTimeFormat('en-ZA', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(entry.generatedDate))}</div></div>)}</div>}</CardContent></Card>
        </>
      )}
    </div>
  )
