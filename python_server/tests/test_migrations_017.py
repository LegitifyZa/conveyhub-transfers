...
class Migration017StaticTests(unittest.TestCase):
    @staticmethod
    def _load_migration() -> str:
        ...
        path = os.path.join(
            ...
            "lib",
            "migrations",
            "017_deedly_party_property_contract_foundation.sql",
        )
    ...
    async def test_existing_person_company_data_survives(self):
        ...
        async def _verify(conn):
            ...
            await query(
                ...
                VALUES ($1, 'TRF-017-EXISTING', '123 Existing St', 100000, 'in_progress', 5)
                """,
                [transfer_id],
                connection=conn,
            )
    ...
    async def test_trust_is_representable(self):
        ...
        async def _verify(conn):
            ...
            await query(
                """
                INSERT INTO transfers (id, transfer_id, property_address, purchase_price, status, accountable_institution_id)
                VALUES ($1, 'TRF-017-TRUST', '123 Trust St', 100000, 'in_progress', 5)
                """,
                [transfer_id],
                connection=conn,
            )
    ...
    async def test_unknown_entity_type_rejected(self):
        ...
        async def _verify(conn):
            ...
            await query(
                """
                INSERT INTO transfers (id, transfer_id, property_address, purchase_price, status, accountable_institution_id)
                VALUES ($1, 'TRF-017-UNKNOWN', '123 Unknown St', 100000, 'in_progress', 5)
                """,
                [transfer_id],
                connection=conn,
            )
    ...
    async def test_arbitrary_entity_type_addable_without_schema_change(self):
        ...
        async def _verify(conn):
            ...
            await query(
                """
                INSERT INTO transfers (id, transfer_id, property_address, purchase_price, status, accountable_institution_id)
                VALUES ($1, 'TRF-017-FUTURE', '123 Future St', 100000, 'in_progress', 5)
                """,
                [transfer_id],
                connection=conn,
            )
    ...
    async def test_multiple_same_role_parties_remain_possible(self):
        ...
        async def _verify(conn):
            ...
            await query(
                """
                INSERT INTO transfers (id, transfer_id, property_address, purchase_price, status, accountable_institution_id)
                VALUES ($1, 'TRF-017-MULTI', '123 Multi St', 100000, 'in_progress', 5)
                """,
                [transfer_id],
                connection=conn,
            )
    ...
    async def test_is_primary_contact_is_just_a_flag(self):
        ...
        async def _verify(conn):
            ...
            await query(
                """
                INSERT INTO transfers (id, transfer_id, property_address, purchase_price, status, accountable_institution_id)
                VALUES ($1, 'TRF-017-PRIMARY', '123 Primary St', 100000, 'in_progress', 5)
                """,
                [transfer_id],
                connection=conn,
            )
    ...
    async def test_matter_properties_input_output_distinction(self):
        ...
        async def _verify(conn):
            ...
            await query(
                ...
                VALUES ($1, 'REF-017-DEV', 'transfer', 'in_progress', 'test-source', 5)
                """,
                [matter_id],
                connection=conn,
            )
    ...
    async def test_multiple_properties_per_matter(self):
        ...
        async def _verify(conn):
            ...
            await query(
                """
                INSERT INTO matters (id, reference_number, matter_type, status, source_record_id, accountable_institution_id)
                VALUES ($1, 'REF-017-MULTI-PROP', 'transfer', 'in_progress', 'test-source', 5)
                """,
                [matter_id],
                connection=conn,
            )
    ...
    async def test_non_deedly_matter_status_unaffected(self):
        ...
        async def _verify(conn):
            ...
            await query(
                """
                INSERT INTO matters (id, reference_number, matter_type, status, source_record_id, accountable_institution_id)
                VALUES ($1, 'REF-017-BOND', 'bond', 'draft', 'test-source', 5)
                """,
                [bond_matter],
                connection=conn,
            )
