...
| SARS V1.17 Path | XSD Type | Cardinality | Phase 3 Rule | DEEDLY Owner | DEEDLY Source | Transformation | Omission Rule | Unresolved Dependency |
|---|---|---|---|---|---|---|---|---|
| /TransferDutyReturn/TDReferenceNo | TaxRefNoType (pattern: \w{8}\|\w{10}) | 1..1 | Required. SARS Transfer Duty reference. | sars_submissions | sars_submissions.sars_reference_no | Validate 8 or 10 word chars; strip whitespace. | Cannot omit; readiness blocker if missing. | How the TDReferenceNo is obtained before/during submission; confirm SARS/Clive flow. |

### Related Exchange

| SARS V1.17 Path | XSD Type | Cardinality | Phase 3 Rule | DEEDLY Owner | DEEDLY Source | Transformation | Omission Rule | Unresolved Dependency |
|---|---|---|---|---|---|---|---|---|
| /TransferDutyReturn/RelatedExchangeDocumentNo | TaxRefNoType (pattern: \w{8}\|\w{10}) | 0..1 | Optional; related exchange transaction. | sars_submissions | sars_submissions.related_exchange_sars_reference_no | Validate 8 or 10 word chars. | Omit if no related exchange. | — |

### Financial Account

| SARS V1.17 Path | XSD Type | Cardinality | Phase 3 Rule | DEEDLY Owner | DEEDLY Source | Transformation | Omission Rule | Unresolved Dependency |
|---|---|---|---|---|---|---|---|---|
| /TransferDutyReturn/FinancialAccount | AccountNoType (pattern: \d{0,10}) | 0..1 | Optional; payment/escrow account. | account_firm_settings | account_firm_settings.sars_contact_details.financial_account or trust account | Digits only, max 10. | Omit if not available. | Which DEEDLY account number SARS expects here. |

### Total Fair Value

| SARS V1.17 Path | XSD Type | Cardinality | Phase 3 Rule | DEEDLY Owner | DEEDLY Source | Transformation | Omission Rule | Unresolved Dependency |
|---|---|---|---|---|---|---|---|---|
| /TransferDutyReturn/TotalFairValueAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional; root-level aggregate. | sars_property_details / sars_calculations | sars_property_details.total_fair_value OR sars_calculations.total_consideration | Decimal 17,2. | Omit if value cannot be sourced reliably. | TotalFairValue semantics: is it the full property value, consideration, or something else? |

### Other Consideration

| SARS V1.17 Path | XSD Type | Cardinality | Phase 3 Rule | DEEDLY Owner | DEEDLY Source | Transformation | Omission Rule | Unresolved Dependency |
|---|---|---|---|---|---|---|---|---|
| /TransferDutyReturn/AnyOtherConsiderationAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_property_details | sars_property_details.other_consideration | Decimal 17,2. | Omit if zero or null. | — |

### TransactionType

| SARS V1.17 Path | XSD Type | Cardinality | Phase 3 Rule | DEEDLY Owner | DEEDLY Source | Transformation | Omission Rule | Unresolved Dependency |
|---|---|---|---|---|---|---|---|---|
| /TransferDutyReturn/TransactionType | DescriptionTextType (maxLength: 100) | 1..1 | Required. Categorises the transaction. | transfers / matters | transfers classification or matter classification | Map to DescriptionTextType (max 100). | Cannot omit; readiness blocker. | DEEDLY matter/transfer classification → SARS TransactionType mapping must be approved by SARS/Clive. |
...
| /TransferDutyReturn/SellersDetails/SellerDetails/IDNo | IDNoType (pattern: \d{13}) | 0..1 | Required for SA individuals when no passport. | Golden Record | Golden Record id_number | Validate 13 digits. | Omit if passport is used. |  |
...
| /TransferDutyReturn/SellersDetails/SellerDetails/PassportCountryCode | CountryType (pattern: \w{0,3}) | 0..1 | Required with PassportNo. | Golden Record | Golden Record passport_country | Map to 3-letter/3-char country code accepted by SARS CountryType. | Omit if RSA ID used. | PassportCountry source values and ISO-3166 alpha-3 mapping. |
...
| /TransferDutyReturn/SellersDetails/SellerDetails/IncomeTaxRefNo | TaxRefNoType (pattern: \w{8}\|\w{10}) | 0..1 | Optional; SA tax reference. | Golden Record | Golden Record tax_number | Validate 8 or 10 word chars. | Omit if NotRegForIncomeTaxInd = Y. | company income-tax reference vs VAT reference source and distinction. |
| /TransferDutyReturn/SellersDetails/SellerDetails/NotRegForIncomeTaxInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | sars_party_details | sars_party_details.not_registered_for_income_tax | bool → Y/N. | Omit if not captured. |  |
| /TransferDutyReturn/SellersDetails/SellerDetails/NotRegisteredReason | NotRegisteredReasonType (enum: MINOR, UNEMPLOYED, EARNING_UNDER_THE_INCOME_TAX_THRESHOLD, DIVORCE_ORDER,  FOREIGN_INDIVIDUAL) | 0..1 | Required when NotRegForIncomeTaxInd = Y. | sars_party_details | sars_party_details.not_registered_reason | Map free-text to NotRegisteredReasonType enum. | Omit if registered. | DEEDLY reason text → SARS NotRegisteredReasonType mapping. |
| /TransferDutyReturn/SellersDetails/SellerDetails/AnnualIncomeAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_party_details | sars_party_details.annual_income | Decimal, totalDigits 17 fractionDigits 2. | Omit if not captured. |  |
| /TransferDutyReturn/SellersDetails/SellerDetails/CountryOfResidence | string (maxLength: 36) | 0..1 | Optional. | Golden Record | Golden Record country_of_residence | Trim to 36 chars. | Omit if unavailable. |  |
| /TransferDutyReturn/SellersDetails/SellerDetails/VATRefNo | TaxRefNoType (pattern: \w{8}\|\w{10}) | 0..1 | Optional; only for VAT-registered entities. | Golden Record | Golden Record vat_number | Validate 8 or 10 word chars. | Omit if not VAT registered. | company VAT number vs income-tax reference source and distinction. |
| /TransferDutyReturn/SellersDetails/SellerDetails/RegistrationNo | OrganisationRegNoType (maxLength: 15) | 0..1 | Optional; company/trust registration. | Golden Record | Golden Record registration_number | Trim to 15 chars. | Omit for natural persons. | company registration number source and whether it is same as TaxRefNo. |
...
| /TransferDutyReturn/SellersDetails/SellerDetails/FixedPeriodYears | decimal (minInclusive: 0; maxInclusive: 999) | 0..1 | Optional. | sars_party_details | sars_party_details.fixed_period_years | Integer 0..999. | Omit if not captured. |  |
| /TransferDutyReturn/SellersDetails/SellerDetails/ConnectedPersonInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | sars_party_details | sars_party_details.is_connected_person | bool → Y/N. | Omit if not captured. |  |
...
| /TransferDutyReturn/SellersDetails/SellerDetails/Gender | GenderType (enum: M, F) | 0..1 | Optional. | Golden Record | Golden Record gender | Map to M/F enum. | Omit for non-natural or if unavailable. | GR gender value set → SARS M/F mapping. |
| /TransferDutyReturn/SellersDetails/SellerDetails/MaritalStatus | MaritalStatusType (enum: N, I, O, D) | 0..1 | Optional. | Golden Record / sars_party_details | Golden Record marital_status or sars_party_details.spouse_details | Map to N/I/O/D enum. | Omit if unavailable. | GR marital-status value set → SARS N/I/O/D mapping. |
...
| /TransferDutyReturn/SellersDetails/SellerDetails/AcquisitionDate | date | 0..1 | Optional. | sars_party_details | sars_party_details.acquisition_date | ISO-8601 date. | Omit if not captured. |  |
...
| /TransferDutyReturn/SellersDetails/SellerDetails/EffectiveDate | date | 0..1 | Optional. | sars_party_details | sars_party_details.effective_date_of_transaction | ISO-8601 date. | Omit if not captured. |  |
...
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/DeedsNo | DeedsNoType (maxLength: 30; minLength: 0) | 0..1 | Optional per party. | Golden Record | Golden Record deeds/registration reference for the party | Pass through; maxLength 30. | Omit if not supplied. | purchaser DeedsNo source and semantics (especially for natural persons). |
...
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/IDNo | IDNoType (pattern: \d{13}) | 0..1 | Required for SA individuals when no passport. | Golden Record | Golden Record id_number | Validate 13 digits. | Omit if passport is used. |  |
...
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/IncomeTaxRefNo | TaxRefNoType (pattern: \w{8}\|\w{10}) | 0..1 | Optional; SA tax reference. | Golden Record | Golden Record tax_number | Validate 8 or 10 word chars. | Omit if NotRegForIncomeTaxInd = Y. | company income-tax reference vs VAT reference source and distinction. |
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/NotRegForIncomeTaxInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | sars_party_details | sars_party_details.not_registered_for_income_tax | bool → Y/N. | Omit if not captured. |  |
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/NotRegisteredReason | NotRegisteredReasonType (enum: MINOR, UNEMPLOYED, EARNING_UNDER_THE_INCOME_TAX_THRESHOLD, DIVORCE_ORDER,  FOREIGN_INDIVIDUAL) | 0..1 | Required when NotRegForIncomeTaxInd = Y. | sars_party_details | sars_party_details.not_registered_reason | Map free-text to NotRegisteredReasonType enum. | Omit if registered. | DEEDLY reason text → SARS NotRegisteredReasonType mapping. |
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/AnnualIncomeAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_party_details | sars_party_details.annual_income | Decimal, totalDigits 17 fractionDigits 2. | Omit if not captured. |  |
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/CountryOfResidence | string (maxLength: 36) | 0..1 | Optional. | Golden Record | Golden Record country_of_residence | Trim to 36 chars. | Omit if unavailable. |  |
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/VATRefNo | TaxRefNoType (pattern: \w{8}\|\w{10}) | 0..1 | Optional; only for VAT-registered entities. | Golden Record | Golden Record vat_number | Validate 8 or 10 word chars. | Omit if not VAT registered. | company VAT number vs income-tax reference source and distinction. |
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/RegistrationNo | OrganisationRegNoType (maxLength: 15) | 0..1 | Optional; company/trust registration. | Golden Record | Golden Record registration_number | Trim to 15 chars. | Omit for natural persons. | company registration number source and whether it is same as TaxRefNo. |
...
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/FixedPeriodYears | decimal (minInclusive: 0; maxInclusive: 999) | 0..1 | Optional. | sars_party_details | sars_party_details.fixed_period_years | Integer 0..999. | Omit if not captured. |  |
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/ConnectedPersonInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | sars_party_details | sars_party_details.is_connected_person | bool → Y/N. | Omit if not captured. |  |
...
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/Gender | GenderType (enum: M, F) | 0..1 | Optional. | Golden Record | Golden Record gender | Map to M/F enum. | Omit for non-natural or if unavailable. | GR gender value set → SARS M/F mapping. |
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/MaritalStatus | MaritalStatusType (enum: N, I, O, D) | 0..1 | Optional. | Golden Record / sars_party_details | Golden Record marital_status or sars_party_details.spouse_details | Map to N/I/O/D enum. | Omit if unavailable. | GR marital-status value set → SARS N/I/O/D mapping. |
...
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/AcquisitionDate | date | 0..1 | Optional. | sars_party_details | sars_party_details.acquisition_date | ISO-8601 date. | Omit if not captured. |  |
...
| /TransferDutyReturn/PurchasersDetails/PurchaserDetails/EffectiveDate | date | 0..1 | Optional. | sars_party_details | sars_party_details.effective_date_of_transaction | ISO-8601 date. | Omit if not captured. |  |
...
| /TransferDutyReturn/ConveyancerDetails/ID | string (maxLength: 10) | 0..1 | Optional; SARS conveyancer ID. | account_firm_settings | account_firm_settings.sars_contact_details.conveyancer_id | Trim to 10 chars. | Omit if not available. | SARS conveyancer identifier source. |
...
| /TransferDutyReturn/EstateAgenciesDetails/EstateAgencyDetails/CommissionPayable | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | transfer_parties role=estate_agent | Not currently stored; may be derived from financials | Decimal 17,2. | Omit if no estate agency. | Estate-agency commission capture location and semantics. |
| /TransferDutyReturn/EstateAgenciesDetails/EstateAgencyDetails/TelNo | TelFaxNoType (pattern: \d{0,15}) | 0..1 | Optional. | Golden Record | GR phone for estate-agent party | Digits, max 15. | Omit if not captured. |  |
| /TransferDutyReturn/EstateAgenciesDetails/EstateAgencyDetails/Surname | SurnameType (maxLength: 53) | 0..1 | Optional. | Golden Record | GR last_name | Trim to 53. | Omit if not captured. |  |
...
| /TransferDutyReturn/EstateAgenciesDetails/EstateAgencyDetails/Initials | InitialsType (maxLength: 5) | 0..1 | Optional. | Golden Record | GR initials | Trim to 5. | Omit if not captured. |  |
| /TransferDutyReturn/EstateAgenciesDetails/EstateAgencyDetails/IncomeTaxNo | TaxRefNoType (pattern: \w{8}\|\w{10}) | 0..1 | Optional. | Golden Record | GR tax_number | 8/10 word chars. | Omit if not captured. |  |
...
| /TransferDutyReturn/PropertyDetails/VATPurposeInd | YesNoIndType (enum: Y, N) | 0..1 | Optional; part of property VAT block. | sars_property_details | sars_property_details.is_enterprise_asset_for_vat | bool → Y/N. | Omit if VAT block not applicable. | SARS VATPurposeInd semantics and mapping from DEEDLY enterprise-asset flag. |
| /TransferDutyReturn/PropertyDetails/InputTaxClaimedInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | sars_property_details | sars_property_details.input_tax_claimed | bool → Y/N. | Omit if VAT block not applicable. | SARS InputTaxClaimedInd semantics. |
| /TransferDutyReturn/PropertyDetails/TransactionDate | date | 0..1 | Optional. | transfers | transfers.transaction_date | ISO-8601 date. | Omit if unavailable. |  |
| /TransferDutyReturn/PropertyDetails/PropertyImprovementInd | IndicatorType (maxLength: 1) | 0..1 | Optional. | sars_property_details | sars_property_details.property_improvement_indicator | Map free-text/char to bt:IndicatorType (length 1). | Omit if not captured. | DEEDLY property_improvement_indicator value set → SARS IndicatorType value. |
| /TransferDutyReturn/PropertyDetails/BoughtByInd | IndicatorType (maxLength: 1) | 0..1 | Optional. | sars_property_details | sars_property_details.bought_by_indicator | Map to single-char IndicatorType. | Omit if not captured. | DEEDLY bought_by_indicator value set → SARS IndicatorType value. |
| /TransferDutyReturn/PropertyDetails/PropertyUsageInd | IndicatorType (maxLength: 1) | 0..1 | Optional. | sars_property_details | sars_property_details.property_usage_indicator | Map to single-char IndicatorType; other value via OtherPropertyUsageDesc. | Omit if not captured. | DEEDLY property_usage_indicator value set → SARS IndicatorType value. |
...
| /TransferDutyReturn/PropertyDetails/PropertyNatureInd | IndicatorType (maxLength: 1) | 0..1 | Optional. | sars_property_details | sars_property_details.property_nature_indicator | Map to single-char IndicatorType; other value via OtherPropertyNatureDesc. | Omit if not captured. | DEEDLY property_nature_indicator value set → SARS IndicatorType value. |
...
| /TransferDutyReturn/PropertyDetails/IncomeTaxActApplicableInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | sars_property_details | sars_property_details.income_tax_act_applicable | Map to Y/N. | Omit if not captured. | DEEDLY income_tax_act_applicable value → Y/N semantics. |
| /TransferDutyReturn/PropertyDetails/LocalValuationAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_property_details / properties | Not currently stored as a discrete field | Decimal 17,2. | Omit if not captured. | LocalValuationAmt data source and semantics. |
| /TransferDutyReturn/PropertyDetails/BondAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | transfer_financials / sars_property_details | Not currently stored as a discrete field | Decimal 17,2. | Omit if not captured. | BondAmt data source and semantics. |
| /TransferDutyReturn/PropertyDetails/PropertyValueAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_property_details / properties | properties market value or municipal valuation | Decimal 17,2. | Omit if not captured. | PropertyValueAmt source (municipal valuation vs market). |
...
| /TransferDutyReturn/PropertyDetails/TotalFairAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_property_details | sars_property_details.total_fair_value | Decimal 17,2. | Omit if not captured. | TotalFairValue: is it aggregate or property-level? Same value appears at root TotalFairValueAmt. |
| /TransferDutyReturn/PropertyDetails/OtherConsiderationAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_property_details | sars_property_details.other_consideration | Decimal 17,2. | Omit if zero/null. |  |
| /TransferDutyReturn/PropertyDetails/TotalConsiderationAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_calculations | sars_calculations.total_consideration | Decimal 17,2. | Omit if not calculated. | TotalConsiderationAmt: same as SellingPrice + OtherConsideration? SARS/Clive semantics. |
...
| /TransferDutyReturn/DutyInterestPayable/PayableAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_calculations | sars_calculations.transfer_duty_payable | Decimal 17,2. | Omit if not calculated. |  |
| /TransferDutyReturn/DutyInterestPayable/NaturalPersons | NaturalPersons wrapper (anonymous sequence) | 0..1 | Optional container for natural-person allocations. | sars_calculations | sars_calculations.party_allocations filtered by allocation_type=natural | Wrap one or more NaturalPerson elements. | Omit if no natural-person allocations. | — |
...
| /TransferDutyReturn/DutyInterestPayable/NonNaturalPersons | NonNaturalPersons wrapper (anonymous sequence) | 0..1 | Optional container for non-natural-person allocations. | sars_calculations | sars_calculations.party_allocations filtered by allocation_type=non_natural | Wrap one or more NonNaturalPerson elements. | Omit if no non-natural-person allocations. | — |
...
| /TransferDutyReturn/DutyInterestPayable/SubTotalPayableAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_calculations | sars_calculations.sub_total | Decimal 17,2. | Omit if not calculated. |  |
| /TransferDutyReturn/DutyInterestPayable/PenaltyInterestAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_calculations | sars_calculations.penalty_interest | Decimal 17,2. | Omit if zero/null. |  |
| /TransferDutyReturn/DutyInterestPayable/TotalPayableAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_calculations | sars_calculations.total_payable | Decimal 17,2. | Omit if not calculated. |  |
...
| /TransferDutyReturn/VATPayable/VATRateInd | string (enum: Z, S) | 0..1 | Optional; required when VAT applies. | sars_calculations | sars_calculations.vat_rate_indicator | Must be Z or S. | Omit when not a VAT transaction. | VATRateInd semantics (Z vs S) and source value set. |
| /TransferDutyReturn/VATPayable/IncludingVATInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | transfers / sars_calculations | Derived from is_vat_transaction and transaction terms | bool → Y/N. | Omit if not a VAT transaction. | Whether purchase price is VAT inclusive. |
| /TransferDutyReturn/VATPayable/VATPayableAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_calculations | sars_calculations.vat_payable | Decimal 17,2. | Omit if not a VAT transaction. | VAT payable calculation semantics and whether it belongs to property or calculation block. |
| /TransferDutyReturn/VATPayable/VATDeclarationPeriod | PeriodType (pattern: \d{2}\|\d{6}) | 0..1 | Optional. | sars_calculations | sars_calculations.calculation_data.vat_declaration_period | Pattern \d{2} or \d{6}. | Omit if not supplied. | VAT declaration period source and format (tax period YYYYMM vs MM). |
| /TransferDutyReturn/VATPayable/OutputTaxPayableAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_calculations | sars_calculations.output_tax_payable | Decimal 17,2. | Omit if not a VAT transaction. | Output tax semantics and source. |
| /TransferDutyReturn/VATPayable/GoingConcernPayableAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 0..1 | Optional. | sars_calculations | sars_calculations.supply_going_concern_payable | Decimal 17,2. | Omit if not a going-concern transaction. | Going-concern treatment and when to populate. |
...
| /TransferDutyReturn/TransactionStatus/Section9Exemption | string (maxLength: 53) | 0..1 | Optional. | sars_submissions | sars_submissions.exemption_section9 | Trim to 53 chars. | Omit if no exemption. |  |
| /TransferDutyReturn/TransactionStatus/AnotherActExemption | string (maxLength: 37) | 0..1 | Optional. | sars_submissions | sars_submissions.exemption_other_act | Trim to 37 chars. | Omit if no exemption. |  |
...
| /TransferDutyReturn/PropertyDescs/PropertyDesc | string (maxLength: 530) | 1..500 | One property description line. | properties | property description or matter property description | Trim to 530 chars. | Omit if not captured. |  |

### VDP Application

| SARS V1.17 Path | XSD Type | Cardinality | Phase 3 Rule | DEEDLY Owner | DEEDLY Source | Transformation | Omission Rule | Unresolved Dependency |
|---|---|---|---|---|---|---|---|---|
| /TransferDutyReturn/VDPApplication | VDPApplicationStructure | 0..1 | Optional container. | sars_submissions | sars_submissions.vdp_* fields | Populate VDPApplicationStructure. | Omit if not a VDP case. | — |
| /TransferDutyReturn/VDPApplication/VDPIndicator | YesNoIndType (enum: Y, N) | 1..1 | Required when VDP block present. | sars_submissions | sars_submissions.vdp_indicator | bool → Y/N. | Omit VDP block when not applicable. |  |
| /TransferDutyReturn/VDPApplication/VDPApplicationNo | VDPApplicationNoType (pattern: \w{0,10}) | 0..1 | Optional. | sars_submissions | sars_submissions.vdp_application_no | Validate \w{0,10}. | Omit if not applicable. |  |
...
| /TransferDutyReturn/SellersDeclarations/SellerDeclaration/Signature | string | 0..1 | Optional; declaration signature. | sars_declaration_events | sars_declaration_events.signature_document_id (pointer to files service) | Currently not a SARS-compatible signature string; emit document reference only if SARS accepts it. | Omit if unsigned. | Signature representation: SARS expects a string here; how DEEDLY supplies a signed declaration without embedding binary. |
...
| /TransferDutyReturn/PurchasersDeclarations/PurchaserDeclaration/Signature | string | 0..1 | Optional. | sars_declaration_events | signature_document_id | Same as seller signature. | Omit if unsigned. | Same as seller signature. |
...
| /TransferDutyReturn/ConveyancerDeclaration/Signature | string | 0..1 | Optional. | sars_declaration_events | signature_document_id | Same as seller signature. | Omit if unsigned. | Same as seller signature. |
...
| SARS V1.17 Path | XSD Type | Cardinality | Phase 3 Rule | DEEDLY Owner | DEEDLY Source | Transformation | Omission Rule | Unresolved Dependency |
|---|---|---|---|---|---|---|---|---|
| /TransferDutyReturn/ReceiptDetails | ReceiptStructure | 0..1 | Container; see children below. | — | — | — | — | — |
| /TransferDutyReturn/ReceiptDetails/ReceiptNo | string (pattern: \d{9}) | 0..1 | Response-side only. | SARS | SARS receipt response | 9 digits. | Omit for outbound. |  |
| /TransferDutyReturn/ReceiptDetails/ReceiptAmt | FinancialAmtDecimalType (totalDigits: 17; fractionDigits: 2) | 1..1 | Response-side only. | SARS | SARS receipt response | Decimal 17,2. | Omit for outbound. |  |
...
| /TransferDutyReturn/AdditionalConveyancerDeclaration/Signature | string | 0..1 | Optional. | sars_declaration_events | signature_document_id | Same as seller signature. | Omit if unsigned. | Same as seller signature. |
...
| SARS V1.17 Path | XSD Type | Cardinality | Phase 3 Rule | DEEDLY Owner | DEEDLY Source | Transformation | Omission Rule | Unresolved Dependency |
|---|---|---|---|---|---|---|---|---|
| /TransferDutyReturn/FormWizard | FormWizardStructure | 1..1 | Required. | transfers / matters / sars_submissions | transfer classification and SARS capture flags | Populate FormWizardStructure. | Cannot omit; readiness blocker. | Classification mapping and many indicator sources, see children. |
| /TransferDutyReturn/FormWizard/TransferDutyType | TransferDutyType (enum: UNIDIVIDED_PROPERTY_TRANSFER, SHARES_MEMBERS_TRANSFER, SERVITUDES) | 0..1 | Optional; identifies wizard type. | transfers / matters | transfers classification | Map to UNIDIVIDED_PROPERTY_TRANSFER / SHARES_MEMBERS_TRANSFER / SERVITUDES. | Omit if classification unavailable. | DEEDLY matter/transfer classification → SARS TransferDutyType mapping. |
| /TransferDutyReturn/FormWizard/NormalInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | transfers | Not captured | bool → Y/N | Omit if not captured. | SARS NormalInd semantics. |
| /TransferDutyReturn/FormWizard/DonationInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | transfers | Not captured | bool → Y/N | Omit if not captured. | Donation transaction flag source. |
| /TransferDutyReturn/FormWizard/ExchangeInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | transfers | Not captured | bool → Y/N | Omit if not captured. | Exchange transaction flag source. |
| /TransferDutyReturn/FormWizard/PatitionInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | transfers | Not captured | bool → Y/N | Omit if not captured. | Partition flag semantics. |
| /TransferDutyReturn/FormWizard/UsurfructInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | sars_party_details | Not captured | bool → Y/N | Omit if not captured. | Usufruct flag source. |
| /TransferDutyReturn/FormWizard/UsurfructType | AcquisitionRenunciationType (enum: ACQUISITION, RENUNCIATION) | 0..1 | Optional. | sars_party_details | Not captured | ACQUISITION/RENUNCIATION | Omit if not captured. | Usufruct type source. |
| /TransferDutyReturn/FormWizard/BaredominiumInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | sars_property_details | Not captured | bool → Y/N | Omit if not captured. | Bare dominium flag source. |
| /TransferDutyReturn/FormWizard/BaredominiumType | AcquisitionRenunciationType (enum: ACQUISITION, RENUNCIATION) | 0..1 | Optional. | sars_property_details | Not captured | ACQUISITION/RENUNCIATION | Omit if not captured. |  |
| /TransferDutyReturn/FormWizard/FideicommissiumInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | sars_party_details | Not captured | bool → Y/N | Omit if not captured. | Fideicommissum flag source. |
| /TransferDutyReturn/FormWizard/FideicommissiumType | AcquisitionRenunciationType (enum: ACQUISITION, RENUNCIATION) | 0..1 | Optional. | sars_party_details | Not captured | ACQUISITION/RENUNCIATION | Omit if not captured. |  |
| /TransferDutyReturn/FormWizard/UsusInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | sars_party_details | Not captured | bool → Y/N | Omit if not captured. | Usus flag source. |
| /TransferDutyReturn/FormWizard/UsusType | AcquisitionRenunciationType (enum: ACQUISITION, RENUNCIATION) | 0..1 | Optional. | sars_party_details | Not captured | ACQUISITION/RENUNCIATION | Omit if not captured. |  |
| /TransferDutyReturn/FormWizard/HabitatioInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | sars_property_details | Not captured | bool → Y/N | Omit if not captured. | Habitatio flag source. |
| /TransferDutyReturn/FormWizard/HabitatioType | AcquisitionRenunciationType (enum: ACQUISITION, RENUNCIATION) | 0..1 | Optional. | sars_property_details | Not captured | ACQUISITION/RENUNCIATION | Omit if not captured. |  |
| /TransferDutyReturn/FormWizard/ResidentialPropertyCompanyTransferInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | transfers | Not captured | bool → Y/N | Omit if not captured. | Residential-property company transfer flag source. |
| /TransferDutyReturn/FormWizard/ExemptInd | YesNoIndType (enum: Y, N) | 0..1 | Optional. | sars_submissions | Derived from exemption fields | bool → Y/N | Omit if not exempt. | ExemptInd source and interplay with TransactionStatus exemptions. |
...
| /TransferDutyReturn/FormWizard/NoOfExistingShareholders | decimal (minInclusive: 0; maxInclusive: 30) | 0..1 | Optional. | transfer_parties | count of role existing_shareholder | Integer 0..30 | Omit if zero. | NoOfExistingShareholders semantics now that V1.17 removed the actual shareholder structures. |
...
| /TransferDutyReturn/FormWizard/NoOfNewShareholders | decimal (minInclusive: 0; maxInclusive: 30) | 0..1 | Optional. | transfer_parties | count of role new_shareholder | Integer 0..30 | Omit if zero. | NoOfNewShareholders semantics now that V1.17 removed the actual shareholder structures. |
...
These fields have a deterministic DEEDLY source and a known XSD constraint; they can be wired as soon as the serializer is rewritten:

- All `SourceSoftware`, `RevisionNo`, `VDPApplication` booleans/numbers.  
- Seller/buyer counts in `FormWizard`.  
- `DutyInterestPayable` totals and `NaturalPersons`/`NonNaturalPersons` allocations from `sars_calculations.party_allocations`.  
- `PropertyDetails/SellingPriceAmt`, `OtherConsiderationAmt`, `MonthlyRentalAmt`, `LandValueAmt`, `OccupationalRentAmt`, `ImprovementValueAmt`, `TotalFairAmt`, `TotalConsiderationAmt` from `sars_property_details` / `sars_calculations` once semantics are fixed.  
...
## 6. Blocked Mappings (require SARS/Clive contract)

- **TransactionType / TransferDutyType**: DEEDLY matter/transfer classification → SARS `DescriptionTextType` / `TransferDutyType` enumeration.  
- **NatureOfPerson**: DEEDLY `entity_type` (`person`/`company`/`trust`) and Golden Record entity kind → SARS `EntityTypeType` enumeration (27 values).  
- **Company tax identifiers**: `IncomeTaxRefNo` vs `VATRefNo` vs `RegistrationNo` source and disambiguation in the Golden Record.  
- **PassportCountry**: `PassportCountryCode` and `SpousePassportCountryCode` country value set and ISO/Alpha-3 mapping to SARS `CountryType`.  
- **Purchaser DeedsNo**: whether a natural person buyer needs a `DeedsNo`, and if so from where.  
- **TotalFairValue**: root `TotalFairValueAmt` vs `PropertyDetails/TotalFairAmt` semantics and which DEEDLY value belongs in each.  
- **Property VAT**: `VATPurposeInd`, `InputTaxClaimedInd`, `VATRateInd`, `IncludingVATInd`, `VATDeclarationPeriod`, `OutputTaxPayableAmt`, `GoingConcernPayableAmt` source values and SARS semantics.  
- **Property indicators**: `PropertyImprovementInd`, `BoughtByInd`, `PropertyUsageInd`, `PropertyNatureInd` single-character value set.  
- **Incomes thresholds / spouse / acquisition fields**: exact Golden Record field names for `AnnualIncomeAmt`, `Gender`, `MaritalStatus`, spouse identity fields.  
- **TDReferenceNo source**: how/when the SARS transfer-duty reference number is produced and stored.  
- **Signature field**: how a signed declaration is represented as an XSD `string` without embedding binary or leaking private data.
...
5. **TransactionType is mandatory and max 100 chars**: Phase 2 did not source it.  
6. **FormWizard is mandatory**: Phase 2 did not emit it.  
7. **Property indicators are single-char `IndicatorType`**: Phase 2 fields are free-text `VARCHAR(20)`.  
8. **`TotalFairValueAmt` appears twice**: once at root and once inside `PropertyDetails`; Phase 2 only stored one value.  
